...
float Data::GET_flow1()//获取传感器1流量
{
    ...
    if(flow_frequency1 != 0)
    {
        ...
        this->flow_1 = this->flow_1 +l_minute1;
...
float Data::GET_flow2()//获取传感器2流量
{
    if(flow_frequency2 != 0)
    {
        ...
        this->flow_2 = this->flow_2 +l_minute2;
...
{
    this->Water_Diffenert = flow_1 - this->flow_2;
...
{
    if(this->Water_Diffenert >= water_diffenert_target)
    {
        digitalWrite(ControlPin_1,HIGH);
        digitalWrite(ControlPin_2,HIGH);
        cb();//其他功能函数
